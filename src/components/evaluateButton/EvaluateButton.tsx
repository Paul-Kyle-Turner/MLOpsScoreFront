...
export const EvaluateButton: React.FC<EvaluateButtonProps> = ({
  ...
  )}&type=${encodeURIComponent(platform.platformType)}`;

  return (
    <Link
      to={evaluationUrl}
      className={`btn btn-${variant} ${size ? `btn-${size}` : ""} ${className} d-flex align-items-center justify-content-center text-decoration-none`}
